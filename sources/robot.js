...
class Robot {
  constructor(longueur,largeur,path){
  ...
  function calcul_position (input) {
    var vec = {};
    vec.x = -Math.cos(Math.PI / 2) * Math.sin(this.Object.rotation[1]) + Math.sin(Math.PI / 2) * Math.cos(this.Object.rotation[1]);
    vec.z = -Math.sin(Math.PI / 2) * Math.sin(this.Object.rotation[1]) - Math.cos(Math.PI / 2) * Math.cos(this.Object.rotation[1]);

    var position = {};
    position.x = this.Object.position[0] - input.longueur * Math.sin(this.Object.rotation[1]) + vec.x * input.largeur;
    position.z = this.Object.position[2] - input.longueur * Math.cos(this.Object.rotation[1]) + vec.z * input.largeur;

    input.Object.setPosition(position.x, 40, position.z);
    input.Object.setRotation(0, this.Object.rotation[1] + input.rotation, 0);
  };
}